...
import com.example.sanmolsoftware.popularmovies.data.api.MoviesApi;
...
import com.example.sanmolsoftware.popularmovies.data.provider.MoviesContract;
...
final class MoviesRepositoryImpl implements MoviesRepository {
    ...
    public Observable<List<Movie>> savedMovies() {
        return mBriteContentResolver.createQuery(MoviesContract.Movies.CONTENT_URI, Movie.PROJECTION, null, null, MoviesContract.Movies.DEFAULT_SORT, true)
    ...
    public Observable<Set<Long>> savedMovieIds() {
        return mBriteContentResolver.createQuery(MoviesContract.Movies.CONTENT_URI, Movie.ID_PROJECTION, null, null, null, true)
    ...
    public void saveMovie(Movie movie) {
        ...
        handler.startInsert(-1, null, MoviesContract.Movies.CONTENT_URI, new Movie.Builder()
                .movie(movie)
                .build());
    }
    ...
    public void deleteMovie(Movie movie) {
        String where = MoviesContract.Movies.MOVIE_ID + "=?";
        String[] args = new String[]{String.valueOf(movie.getId())};

        AsyncQueryHandler handler = new AsyncQueryHandler(mContentResolver) {};
        handler.startDelete(-1, null, MoviesContract.Movies.CONTENT_URI, where, args);
    }
